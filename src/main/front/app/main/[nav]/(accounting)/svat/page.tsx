...
import {getAllSalesVATApi} from "@/features/accounting/api/search-server-api";
...
export default async function SvatPage({searchParams}: PageByProps) {
    ...
    const initialSalesVatList = await getAllSalesVATApi()

    return (
        <AccountingSearch
            companyList={companyList}
            division='svat'
            initialListState={initialSalesVatList}
            page={page}/>
    )
}