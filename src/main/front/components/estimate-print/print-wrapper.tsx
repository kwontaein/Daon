    ...
    useEffect(()=>{
        ...
        if(receipts){
            ...
            items = estimate.items.map((item:ResponseEstimateItem)=>{
                ...
                return {productName,modelName,quantity,unitPrice, totalPrice, memo, estimateDate:estimate.estimateDate}
            })
            setEstimateData({
                items,
        ...
        <section className="print-estimate-container">
            <div className="print-estimate-header">
                ...
                <div className="print-estimate-left">
                    {receipts &&
                        <select onChange={(e:ChangeEvent<HTMLSelectElement>)=>setEstimateData({company: JSON.parse(e.target.value)})}>
                            {companyList.map((company)=>(
                                <option key={company.companyId} value={JSON.stringify(company)}>{company.companyName}</option>
                            ))}
                        </select>
                    }
                    <select onChange={(e:ChangeEvent<HTMLSelectElement>)=>setEstimateData({title: e.target.value})}>
                    ...
                    <div className="checkbox-container">
                        <label>
                            <input type="checkbox" checked={estimateData.isDatePrint} onChange={()=>setEstimateData({isDatePrint:!estimateData.isDatePrint})}/>
                            일자인쇄
                        </label>
                    </div>
                    <div className="checkbox-container">
                        <label>
                            <input type="checkbox" checked={estimateData.isStamp} onChange={()=>setEstimateData({isStamp:!estimateData.isStamp})}/>
                            도장인쇄
                        </label>
                    </div>
                    <div className="checkbox-container">
                        <label>
                            <input type="checkbox" checked={estimateData.isMemoToDate} onChange={()=>setEstimateData({isMemoToDate:!estimateData.isMemoToDate})}/>
                            비고란 날짜인쇄
                        </label>
                    </div>
                </div>
            ...
            <article className="estiamte-content-wrapper">
