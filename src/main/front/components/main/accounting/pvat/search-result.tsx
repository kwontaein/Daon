...
export default function PVATSearchResult({purchaseVATList}: { purchaseVATList: PurchaseVAT[] }) {
                ...
                <table className={`search-result-table  ${mode !== 'pc' ? 'mobile' : ''}`}>
                    ...
                    {mode === 'pc' ?
                        ...
                        {purchaseVATList.map((purchaseVAT) => (
                            <tr key={purchaseVAT.purchaseVATId} ref={(el) => {
                                itemsRef.current[purchaseVAT.purchaseVATId] = el
                            }} className={target === purchaseVAT.purchaseVATId ? 'is-click' : ''}>
                                ...
                                <td className='icon'
                                    onClick={() => target === purchaseVAT.purchaseVATId ? setTarget(null) : setTarget(purchaseVAT.purchaseVATId)}>
                                    <MemoizedFontAwesomeIcon icon={faEllipsis}
                                                             style={target === purchaseVAT.purchaseVATId && {color: 'orange'}}/>
                                    {target === purchaseVAT.purchaseVATId &&
                                        <AccountingOptions id={purchaseVAT.purchaseVATId} division='pvat'/>}
                                </td>
                            </tr>
                        ))}
                        </tbody>
                        :
                        purchaseVATList.map((purchaseVAT) => (
                            <tbody key={purchaseVAT.purchaseVATId} ref={(el) => {
                                itemsRef.current[purchaseVAT.purchaseVATId] = el
                            }} className={target === purchaseVAT.purchaseVATId ? 'is-click' : ''}>
                            <tr>
