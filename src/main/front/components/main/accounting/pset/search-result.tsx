                ...
                <table className={`search-result-table  ${mode !== 'pc' ? 'mobile' : ''}`}>
                    ...
                    {mode === 'pc' ?
                        ...
                        {procurements.map((pset) => (
                            <tr key={pset.procurementSettlementId} ref={(el) => {
                                itemsRef.current[pset.procurementSettlementId] = el
                            }} className={target === pset.procurementSettlementId ? 'is-click' : ''}>
                                ...
                                <td>{pset.memo}</td>
                                <td className='icon'
                                    onClick={() => target === pset.procurementSettlementId ? setTarget(null) : setTarget(pset.procurementSettlementId)}>
                                    <MemoizedFontAwesomeIcon icon={faEllipsis}
                                                             style={target === pset.procurementSettlementId && {color: 'orange'}}/>
                                    {target === pset.procurementSettlementId &&
                                        <AccountingOptions id={pset.procurementSettlementId} division='pset'/>}
                                </td>
                            </tr>
                        ))}
                        </tbody>
                        :
                        procurements.map((pset) => (
                            <tbody key={pset.procurementSettlementId} ref={(el) => {
                                itemsRef.current[pset.procurementSettlementId] = el
                            }} className={target === pset.procurementSettlementId ? 'is-click' : ''}>
                            <tr>
                                <td>{pset.categorySelection}</td>
