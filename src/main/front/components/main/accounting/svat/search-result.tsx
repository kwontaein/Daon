...
export default function SVATSearchResult({salesVATList}: { salesVATList: SalesVAT[] }) {
            ...
            {mode &&
                <table className={`search-result-table  ${mode !== 'pc' ? 'mobile' : ''}`}>
                    <colgroup>
                        ...
                        <col style={{
                            width: '5%',
                            minWidth: `${mode === 'pc' ? '63px' : (mode === 'tabelt' ? '100px' : '110px')}`
                        }}/>
                    ...
                    {mode === 'pc' ?
                        ...
                        {salesVATList.map((salesVat) => (
                            <tr key={salesVat.salesVATId} ref={(el) => {
                                itemsRef.current[salesVat.salesVATId] = el
                            }} className={target === salesVat.salesVATId ? 'is-click' : ''}>
                                ...
                                <td className='icon'
                                    onClick={() => target === salesVat.salesVATId ? setTarget(null) : setTarget(salesVat.salesVATId)}>
                                    <MemoizedFontAwesomeIcon icon={faEllipsis}
                                                             style={target === salesVat.salesVATId && {color: 'orange'}}/>
                                    {target === salesVat.salesVATId &&
                                        <AccountingOptions id={salesVat.salesVATId} division='svat'
                                                           paidDate={salesVat.paidDate}/>}
                                </td>
                            </tr>
                        ))}
                        </tbody>
                        :
                        salesVATList.map((salesVat) => (
                            <tbody key={salesVat.salesVATId} ref={(el) => {
                                itemsRef.current[salesVat.salesVATId] = el
                            }} className={target === salesVat.salesVATId ? 'is-click' : ''}>
                            <tr>
