    ...
    return (
        <table className="search-result-table">
            ...
            {pageByCustomers.map((customer: ResponseCustomer, index) => (
                <tr key={index} ref={(el) => {
                    itemsRef.current[customer.customerId] = el
                }} className={target === customer.customerId ? 'is-click' : ''}>
                    ...
                    <td></td>
                    <td className='icon'
                        onClick={() => target === customer.customerId ? setTarget(null) : setTarget(customer.customerId)}>
                        <MemoizedFontAwesomeIcon icon={faEllipsis}
                                                 style={target === customer.customerId && {color: 'orange'}}/>
