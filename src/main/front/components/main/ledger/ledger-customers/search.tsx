...
export default function LedgerCustomerListSearch(){
    ...
    const changeHandler = useCallback(<T extends Record<string, string>[]>(customerList: T) => {
        if (formRef.current) {
            ...
            const customerName = customerList.map(({customerName})=>customerName)
            console.log(customerIds.toString(),customerName.toString())
            formData.set('customerIds' ,JSON.stringify(customerIds));
            formData.set('customerName' ,customerName.toString());
