...
export default function LedgerEtcSearchResult({searchInfo}: {
    searchInfo: { searchResult: ResponseLedger[], searchTitle: string }
}) {
            ...
            <table className='search-result-table'>
                ...
                {searchInfo.searchResult.map((ledger) => (
                    <tr key={ledger.receiptId}>
                        ...
                        <td>{`${ledger.productName ?? ""}${(ledger.productName ?? "") && '[' + (ledger.modelName ?? '-') + ']'}`}</td>
                        <td>{ledger.description}</td>
                        <td>{(ledger.quantity ?? 0).toLocaleString('ko-KR')}</td>
                        <td className='right-align'>{(ledger.outPrice ?? 0).toLocaleString('ko-KR')}</td>
                        <td className='right-align'>{(ledger.totalPrice ?? 0).toLocaleString('ko-KR')}</td>
                    </tr>
                ))}
                <tr className='none-hover' style={{borderBottom: 'none'}}>
                    ...
                    <td></td>
                    <td></td>
                    <td><b>{(totalResult.quantity ?? 0).toLocaleString('ko-KR')}</b></td>
                    <td></td>
                    <td className='right-align'><b>{(totalResult.total ?? 0).toLocaleString('ko-KR')}</b></td>
                </tr>
                </tbody>
            </table>
        </>
